...
import { api, onAgentApproval, onAgentPlan, onAgentQuestion, onAiChatOutput, } from "../lib/tauri";
...
import { contextUsageFromMessages } from "../lib/streamStats";
import { appendTextDelta, applyActivityEvent, flattenActivity, textFromSegments, } from "./turnSegments";
import { enqueueMessage, removeQueuedMessage, takeNextQueued, updateQueuedMessage, } from "./messageQueue";
import { cancelSpeech, currentSpeechEpoch, enqueueSpeechBytes, speak, speakBytes, speakableText, } from "../lib/voice";
...
async function speakSentenceQueued(raw) {
    const v = useVoiceStore.getState();
    const text = speakableText(raw);
    if (!v.ttsEnabled || !text.trim())
        return;
    ...
    const enqueue = (b64, mime) => {
        if (currentSpeechEpoch() !== epoch)
            return; // stopped / barged-in mid-synth → discard
...
import { estimateTokens, liveGenerationStats, liveTokenStats, sessionCostFromMessages, } from "../lib/streamStats";
/** Format execution duration cleanly e.g. "Worked for 42s", "Worked for 2m 15s", "Worked for 1h 12m 4s". */
export function formatWorkingDuration(ms, prefix = "Worked for") {
...
import { resolveEffectiveMode } from "../lib/agentMode";
...
export const useAgentStore = create((set, get) => ({
    ...
    planRevising: false,
    planMode: typeof localStorage !== "undefined" &&
        localStorage.getItem("xconsole-agent-plan-mode") === "1",
    agentMode: (typeof localStorage !== "undefined" &&
        localStorage.getItem("xconsole-agent-mode")) ||
        "auto",
    hydrated: false,
    ...
    init: async () => {
        ...
        try {
            ...
            const openId = lastId && list.some((c) => c.id === lastId) ? lastId : list[0]?.id;
            if (openId) {
    ...
    togglePlanMode: () => set((s) => {
        const nextPlan = !s.planMode;
        const agentMode = nextPlan ? "plan" : "auto";
        try {
    ...
    subscribeApprovals: async () => {
        ...
        const unPlan = await onAgentPlan((plan) => {
            ...
            void notify("xConsole agent — plan ready for review", plan.title || "Review the proposed plan.");
        });
        return () => {
            unApproval();
            unQuestion();
    ...
    openConversation: async (id) => {
        ...
        if (conv.targets_json) {
            try {
                targets = JSON.parse(conv.targets_json);
            }
            catch {
                targets = [];
            }
        ...
        const initialContextUsage = contextUsageFromMessages(messages, activeProvider?.kind ?? undefined, activeModel ?? undefined);
        set({
            sessionId: id,
            messages,
            targets,
    ...
    enqueueOrSend: (text, images) => {
        ...
        if (get().tryRouteChatToPending(trimmed))
            return;
        if (get().streaming) {
    ...
    send: async (text, opts) => {
        ...
        if (trimmed && get().tryRouteChatToPending(trimmed))
            return;
        if (get().streaming)
            return;
        const userMsg = {
        ...
        const unlisten = await onAiChatOutput(mySession, (ev) => {
            ...
            if (ev.kind === "Cost") {
                ...
                if (latestStats)
                    latestStats = { ...latestStats, costUsd };
                if (isCurrent()) {
                    set((s) => ({
            ...
            if (ev.kind === "ConversationCompacted") {
                compactionMarker = {
                    ...
                    isCompaction: true,
                    compactionTokensBefore: ev.data?.tokens_before,
                    compactionTokensAfter: ev.data?.tokens_after,
                    compactionPrunedTools: ev.data?.pruned_tools,
                };
                if (isCurrent()) {
                    set((s) => ({
            ...
            if (isCurrent())
                set({ activity: turnActivity, streamingSegments: turnSegments });
        });
        try {
            ...
            if (isCurrent()) {
                ...
                if (streamVoice && streamingSpoke) {
                    ...
                    if (tail)
                        speechChain = speechChain.then(() => speakSentenceQueued(tail));
                }
                else {
            ...
            const hasAssistantContent = Boolean(turnText.trim()) ||
                turnActivity.length > 0 ||
                turnSegments.length > 0;
            const assistantMsg = hasAssistantContent
                ? {
                    role: "assistant",
                    content: turnText,
                    activity: turnActivity.length > 0 ? [...turnActivity] : undefined,
    ...
    retryLast: async () => {
        ...
        const hasAssistantWork = lastMsg.role === "assistant" &&
