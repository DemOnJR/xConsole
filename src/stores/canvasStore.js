...
import { applyRowCounts, autoLayout, computeBoxes, isSolo, layoutFromColumnCounts, moveToRow, moveWithinRow, reconcile, resizeRow, resizeSolo, rowsFromPositions, resizeTile, toggleFullWidth, } from "../lib/tileLayout";
 ...
 */
function applyTiles(s, dims) {
    if (s.nodes.length === 0)
        return { tileLayout: null };
...
export const useCanvasStore = create()(persist((set, get) => ({
    ...
    onNodesChange: (changes) => set((s) => {
        ...
        if (s.layoutMode === "tile" && s.tileLayout && s.paneSize) {
            ...
            const resizes = meaningful.filter((c) => c.type === "dimensions" && !!c.dimensions && c.resizing === true);
            if (resizes.length > 0) {
                ...
                for (const c of resizes) {
                    ...
                    if (!node)
                        continue;
                    const dw = c.dimensions.width - (node.width ?? 0);
                    const dh = c.dimensions.height - (node.height ?? 0);
                    ...
                    if (isSolo(layout)) {
                        ...
                        layout = resizeSolo(layout, Math.abs(dw) >= 1 ? dw / s.paneSize.width : 0, Math.abs(dh) >= 1 ? dh / s.paneSize.height : 0);
                        continue;
                    }
                    if (layout.tree) {
                        layout = resizeTree(layout, c.id, Math.abs(dw) >= 1 ? (dw / s.paneSize.width) * 2 : 0, Math.abs(dh) >= 1 ? (dh / s.paneSize.height) * 2 : 0);
                        continue;
                    }
                    if (Math.abs(dw) >= 1) {
    ...
    onEdgesChange: (changes) => {
        ...
        set((s) => {
            ...
            if (removedIds.length > 0) {
                ...
                const unlinkedSftpIds = new Set(removed.map((e) => e.source).filter(Boolean));
                if (unlinkedSftpIds.size > 0) {
                    nodes = s.nodes.map((n) => unlinkedSftpIds.has(n.id) && n.type === "sftp"
                        ? {
                            ...n,
                            data: {
                                ...n.data,
    ...
    onConnect: (connection) => {
        ...
        if (!sftpNode || sftpNode.type !== "sftp")
            return;
        ...
        set((s) => ({
            ...
            nodes: s.nodes.map((n) => n.id === sftpId
                ? {
                    ...n,
                    data: {
                        ...n.data,
    ...
    updateNodeData: (id, partial) => set((s) => ({
        nodes: s.nodes.map((n) => n.id === id ? { ...n, data: { ...n.data, ...partial } } : n),
    })),
    addVps: (vps, position) => {
    ...
    toggleFillPane: (id) => {
        ...
        const fillsPane = pane && w >= pane.width - 4 && h >= pane.height - 4 && node.position.x <= 4 && node.position.y <= 4;
        if (fillsPane) {
            set((s) => ({
    ...
    addGoal: (goalId, position) => {
        ...
        if (agent && pane) {
            ...
            if (w >= pane.width - 4 && h >= pane.height - 4) {
                if (get().layoutMode === "tile") {
                    ...
                    set((s) => ({
                        nodes: s.nodes.map((n) => n.id === agent.id
                            ? { ...n, position: { x: 80, y: 80 }, width: NODE_W, height: NODE_H }
                            : n),
                    }));
                }
        ...
        const existing = get().nodes.find((n) => n.type === "goal" && String(n.data.goalId) === goalId);
        if (existing) {
            get().focus(existing.id);
            return existing.id;
        }
    ...
    removeNode: (id) => {
        set((s) => {
            ...
            return {
                ...
                nodes: s.nodes
                    .filter((n) => n.id !== id)
                    .map((n) => n.type === "sftp" && n.data.linkedTerminalId === id
    ...
    setLayout: (mode) => {
        ...
        if (mode === "tile")
            get().retileFromPositions();
    },
    focus: (id) => set((s) => {
        if (!id)
            return { focusedId: null };
        const webglIds = [id, ...s.webglIds.filter((w) => w !== id)].slice(0, MAX_WEBGL);
        return { focusedId: id, webglIds };
    }),
    isWebgl: (id) => get().webglIds.includes(id),
    arrangeTiles: (dims) => set((s) => applyTiles(s, dims ?? s.paneSize ?? undefined)),
    setPaneSize: (dims) => set((s) => {
        if (s.paneSize &&
    ...
    retileFromPositions: (dims) => set((s) => {
        if (s.nodes.length === 0)
            return {};
        const derived = rowsFromPositions(s.nodes.map((n) => ({
            id: n.id,
            x: n.position.x,
            y: n.position.y,
    ...
    setTileColumns: (counts) => set((s) => {
        const ids = s.nodes.map((n) => n.id);
        return applyTiles({ ...s, tileLayout: layoutFromColumnCounts(ids, counts) }, s.paneSize ?? undefined);
    }),
    resetTileLayout: () => set((s) => applyTiles({ ...s, tileLayout: autoLayout(s.nodes.map((n) => n.id)) }, s.paneSize ?? undefined)),
    moveTile: (id, dir, axis) => set((s) => {
        const base = reconcile(s.tileLayout, s.nodes.map((n) => n.id));
    ...
    growTile: (id, delta, axis) => set((s) => {
        const base = reconcile(s.tileLayout, s.nodes.map((n) => n.id));
        const next = base.tree
            ? resizeTree(base, id, axis === "horizontal" ? delta : 0, axis === "vertical" ? delta : 0)
            : axis === "horizontal"
                ? resizeTile(base, id, delta)
                : resizeRow(base, id, delta);
        return applyTiles({ ...s, tileLayout: next }, s.paneSize ?? undefined);
    }),
    toggleTileFullWidth: (id) => set((s) => {
        const base = reconcile(s.tileLayout, s.nodes.map((n) => n.id));
        const next = toggleFullWidth(base, id);
        return applyTiles({ ...s, tileLayout: { ...next, tree: treeOf(next) } }, s.paneSize ?? undefined);
    }),
    clear: () => set({ nodes: [], edges: [], webglIds: [], focusedId: null, tileLayout: null }),
    queueTerminalCommand: (nodeId, command, send) => set((s) => ({
        pendingTerminalCommands: {
