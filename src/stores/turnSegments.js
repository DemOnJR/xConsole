export function appendTextDelta(segments, delta) {
    if (!delta)
        return segments;
...
export function applyActivityEvent(segments, ev) {
    const ids = eventTargetIds(ev);
    if (ids.length > 0) {
        const si = segments.findIndex((s) => s.type === "activity" && s.items.some((item) => itemMatchesIds(item, ids)));
        if (si >= 0) {
...
export function flattenActivity(segments) {
...
export function segmentsFromMessage(message) {
...
function eventTargetIds(ev) {
    if (ev.kind === "ToolCall")
        return [ev.data.id];
    if (ev.kind === "ToolResult")
        return [ev.data.id];
    if (ev.kind === "Status" && /parallel/i.test(ev.data))
...
export function applyStreamEvent(activity, ev) {
    switch (ev.kind) {
        ...
        case "ToolCall":
            if (activity.some((a) => a.id === ev.data.id))
                return activity;
            if (/mcp/i.test(ev.data.name))
                return activity;
            return [
                ...activity,
                {
        ...
        case "ToolResult": {
            if (ev.data.id.startsWith("snapshot-"))
                return activity;
            ...
            const stillRunning = next.some((a) => a.state === "running" && a.id !== "parallel-batch" && a.kind !== "status");
            if (!stillRunning) {
                next = next.map((a) => a.id === "parallel-batch" && a.state === "running"
                    ? {
                        ...a,
                        state: "done",
        ...
        case "Activity": {
            ...
            switch (d.type) {
                ...
                case "ToolEnd": {
                    const endState = d.data.ok ? "done" : "error";
                    const afterEnd = activity.map((a) => {
                        if (a.id !== d.data.id && !a.id.startsWith(`${d.data.id}-`))
                            return a;
                        if (a.kind === "file_edit") {
                    ...
                    const stillRunning = afterEnd.some((a) => a.state === "running" && a.id !== "parallel-batch" && a.kind !== "status");
                    if (!stillRunning) {
                        return afterEnd.map((a) => a.id === "parallel-batch" && a.state === "running"
                            ? { ...a, state: "done", label: a.label.replace(/…$/, " — done") }
                            : a);
                    }
                    return afterEnd;
                }
...
//# sourceMappingURL=turnSegments.js.map