...
export function detectEncoding(bytes: Uint8Array): string {
  ...
  for (let i = 0; i < bytes.length; i += 1) {
    ...
    if (b >= 0xc0 && b <= 0xff) cyrillicScore += 1;
    // CP1256 Arabic letters: 0xC1..0xFE
    if (b >= 0xc1 && b <= 0xfe) arabicScore += 1;
...
const CP1251_MAP: Record<string, number> = {
...
export function checkEncodingLoss(
  text: string,
  targetEncoding: string,
): { hasLoss: boolean; lostChars: string[] } {
