...
export type Split =
  | { kind: "leaf"; id: string; weight: number }
  | { kind: "row"; weight: number; kids: Split[] }
  | { kind: "col"; weight: number; kids: Split[] };

const clampW = (w: number) =>
  Math.min(MAX_WEIGHT, Math.max(MIN_WEIGHT, Number.isFinite(w) ? w : 1));

export function leaf(id: string, weight = 1): Split {
  return { kind: "leaf", id, weight: clampW(weight) };
}

export function treeIds(node: Split): string[] {
  if (node.kind === "leaf") return [node.id];
  return node.kids.flatMap(treeIds);
}

export function cloneSplit(node: Split): Split {
...
export function prune(node: Split | null): Split | null {
  if (!node) return null;
  if (node.kind === "leaf") return node;
  const kids = node.kids.map(prune).filter((k): k is Split => k !== null);
  if (kids.length === 0) return null;
  if (kids.length === 1) return { ...kids[0], weight: node.weight };
  return { kind: node.kind, weight: clampW(node.weight), kids };
}

export function treeFromIdsRow(ids: string[]): Split {
  if (ids.length === 0) return { kind: "row", weight: 1, kids: [] };
...
export function autoTree(ids: string[]): Split {
  if (ids.length === 0) return { kind: "col", weight: 1, kids: [] };
  if (ids.length === 1) return leaf(ids[0]);
...
function groupsOnAxis(nodes: PlacedNode[], axis: "x" | "y"): PlacedNode[][] {
  const start = (n: PlacedNode) => (axis === "x" ? n.x : n.y);
  const end = (n: PlacedNode) => (axis === "x" ? n.x + n.width : n.y + n.height);
 ...
 */
export function treeFromPositions(nodes: PlacedNode[]): Split {
  if (nodes.length === 0) return { kind: "col", weight: 1, kids: [] };
  if (nodes.length === 1) return leaf(nodes[0].id);

  const xGroups = groupsOnAxis(nodes, "x");
  if (xGroups.length >= 2) {
    const totalW = xGroups.reduce(
      (s, g) => s + Math.max(...g.map((n) => n.x + n.width)) - Math.min(...g.map((n) => n.x)),
      0,
    );
    return {
      kind: "row",
      ...
      kids: xGroups.map((g) => {
        const span =
          Math.max(...g.map((n) => n.x + n.width)) - Math.min(...g.map((n) => n.x));
  ...
  if (yGroups.length >= 2) {
    const totalH = yGroups.reduce(
      (s, g) => s + Math.max(...g.map((n) => n.y + n.height)) - Math.min(...g.map((n) => n.y)),
      0,
    );
    return {
      kind: "col",
      ...
      kids: yGroups.map((g) => {
        const span =
          Math.max(...g.map((n) => n.y + n.height)) - Math.min(...g.map((n) => n.y));
        const child = treeFromPositions(g);
...
export function computeTreeBoxes(
  node: Split,
  x: number,
  y: number,
  width: number,
...
export function treeOf(layout: TileLayout): Split {
  ...
  if (layout.rows.length === 0) return { kind: "col", weight: 1, kids: [] };
...
function mapLeaves(node: Split, fn: (id: string) => string): Split {
  if (node.kind === "leaf") return { ...node, id: fn(node.id) };
  return { ...node, kids: node.kids.map((k) => mapLeaves(k, fn)) };
}

/** Swap two windows. Every other tile stays put. */
export function swapLeaves(layout: TileLayout, a: string, b: string): TileLayout {
...
function removeLeaf(node: Split, id: string): Split | null {
  if (node.kind === "leaf") return node.id === id ? null : node;
  const kids = node.kids.map((k) => removeLeaf(k, id)).filter((k): k is Split => k !== null);
  if (kids.length === 0) return null;
  if (kids.length === 1) return { ...kids[0], weight: node.weight };
  return { kind: node.kind, weight: node.weight, kids };
}

function replaceLeaf(node: Split, id: string, next: Split): Split {
  if (node.kind === "leaf") return node.id === id ? { ...next, weight: node.weight } : node;
...
export function dockLeaf(
  ...
  const pairKind: "row" | "col" = edge === "left" || edge === "right" ? "row" : "col";
...
export function dockToPane(layout: TileLayout, dragged: string, edge: DockEdge): TileLayout {
  let tree = removeLeaf(treeOf(layout), dragged);
  const moved = leaf(dragged);
  if (!tree) return layoutFromTree(moved, layout.fillW, layout.fillH);
  const pairKind: "row" | "col" = edge === "left" || edge === "right" ? "row" : "col";
  const first = edge === "left" || edge === "top";
  const root: Split = {
    kind: pairKind,
...
export function reconcileTree(tree: Split | null, ids: string[]): Split {
  if (!tree) return autoTree(ids);
  const live = new Set(ids);
  const stripped = prune(
    (function strip(n: Split): Split | null {
      if (n.kind === "leaf") return live.has(n.id) ? n : null;
      const kids = n.kids.map(strip).filter((k): k is Split => k !== null);
      if (kids.length === 0) return null;
      if (kids.length === 1) return { ...kids[0], weight: n.weight };
...
export function resizeTree(
  ...
  function walk(node: Split, _axis: "row" | "col" | null): boolean {
    ...
    if (node.kids.length < 2) return true;
...
export function moveInTree(
  ...
  const want: "row" | "col" = axis === "horizontal" ? "row" : "col";
  const tree = cloneSplit(treeOf(layout));

  function walk(node: Split): boolean {
    if (node.kind === "leaf") return node.id === id;
    const idx = node.kids.findIndex(walk);
    if (idx < 0) return false;
    if (node.kind === want) {
      const to = idx + dir;
      if (to < 0 || to >= node.kids.length) return true;
...
export function dropTargetAt(
  ...
  if (paneLeft) return { kind: "pane", edge: "left", x: 0, y: 0, width: paneW * 0.35, height: paneH };
  if (paneRight) return { kind: "pane", edge: "right", x: paneW * 0.65, y: 0, width: paneW * 0.35, height: paneH };
  if (paneTop) return { kind: "pane", edge: "top", x: 0, y: 0, width: paneW, height: paneH * 0.3 };
  if (paneBottom) return { kind: "pane", edge: "bottom", x: 0, y: paneH * 0.7, width: paneW, height: paneH * 0.3 };
  return null;
}

export function applyDrop(layout: TileLayout, draggedId: string, drop: DropTarget): TileLayout {
  if (drop.kind === "swap" && drop.targetId) return swapLeaves(layout, draggedId, drop.targetId);
  if (drop.kind === "dock" && drop.targetId && drop.edge) {
    return dockLeaf(layout, draggedId, drop.targetId, drop.edge);
  }
  if (drop.kind === "pane" && drop.edge) return dockToPane(layout, draggedId, drop.edge);
  return layout;
}

export function fillOfTree(layout: TileLayout) {
  return {
...
export type SavedSplit =
  | { kind: "leaf"; index: number; weight: number }
  | { kind: "row" | "col"; weight: number; kids: SavedSplit[] };

export function serializeSplit(node: Split, indexOf: (id: string) => number): SavedSplit | null {
  if (node.kind === "leaf") {
  ...
  const kids = node.kids
    ...
    .filter((k): k is SavedSplit => k !== null);
  if (kids.length === 0) return null;
  return { kind: node.kind, weight: node.weight, kids };
}

export function deserializeSplit(saved: SavedSplit, ids: string[]): Split | null {
  if (saved.kind === "leaf") {
    const id = ids[saved.index];
    if (!id) return null;
    return leaf(id, saved.weight);
  }
  if (!Array.isArray(saved.kids)) return null;
  const kids = saved.kids
    .map((k) => deserializeSplit(k, ids))
    .filter((k): k is Split => k !== null);
  if (kids.length === 0) return null;
