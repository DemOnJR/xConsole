 ...
 */
export function cacheBreakdown(stats) {
...
export function emptySessionCache() {
    return { hit: 0, miss: 0, turns: 0, rate: 0 };
}
export function addTurnToSessionCache(acc, stats) {
...
export function sessionCacheFromMessages(messages) {
    return messages.reduce((acc, message) => addTurnToSessionCache(acc, message.tokenStats), emptySessionCache());
}
export function sessionCostFromMessages(messages) {
...
export function displayTurnStats(live, last) {
...
export function formatSessionCache(acc) {
    if (acc.turns <= 0)
        return "";
...
export function formatCacheTooltip(stats, session, costUsd) {
    const lines = [];
    if (stats && stats.tokensPerSec > 0)
        lines.push(formatTokensPerSec(stats.tokensPerSec));
    ...
    if (cache) {
        lines.push(`${formatTokenCount(cache.hit)} hit · ${formatTokenCount(cache.miss)} miss · ${Math.round(cache.rate * 100)}%`);
    }
    const sessionLine = session ? formatSessionCache(session) : "";
    if (sessionLine)
        lines.push(sessionLine);
    if (costUsd && costUsd > 0)
        lines.push(`$${costUsd.toFixed(4)}`);
    return lines.join("\n");
}
...
export function liveTokenStats(text, startedAtMs) {
    return liveGenerationStats(text, startedAtMs, 0);
}
/** tok/s for the current generation burst only (excludes earlier tool-wait time). */
export function liveGenerationStats(fullText, burstStartedAtMs, tokensBeforeBurst) {
...
export function defaultContextLimit(kind, model) {
...
export function contextUsageFromMessages(messages, providerKind, model) {
    ...
    for (const m of messages) {
        ...
        if (m.toolCalls && Array.isArray(m.toolCalls)) {
            for (const tc of m.toolCalls) {
                const argStr = typeof tc.arguments === "string"
                    ? tc.arguments
    ...
    if (estimatedConversationTokens === 0)
        return null;
