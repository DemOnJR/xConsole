...
function hasWriteOrSubstitution(command) {
...
export function isReadOnly(command) {
    if (hasWriteOrSubstitution(command))
        return false;
    ...
    for (const rawSeg of segments) {
        ...
        if (token === "find" && tokens.some((t) => FIND_WRITE_PREDICATES.has(t)))
            return false;
        if (!READ_ONLY.has(token)) {
            const lc = seg.toLowerCase();
            const statusOk = lc.startsWith("systemctl status") ||
...
function isTerraformReadonly(command) {
    const lc = command.toLowerCase();
    if (lc.includes("terraform apply") ||
    ...
    return (lc.startsWith("tfc remote plan") ||
...
export function isAllowlisted(command) {
    return ((isReadOnly(command) || isTerraformReadonly(command)) && !touchesSensitivePath(command));
}
/** The effective safety mode for a vps (global setting + per-vps override). */
export function effectiveMode(globalMode, vpsId, perVps) {
...
export function shouldAutoRun(mode, command) {
    if (mode === "full")
        return true;
    if (mode === "allowlist")
