import { computeTreeBoxes, fillOfTree, reconcileTree, treeFromPositions, treeOf, } from "./tileTree";
...
const clampWeight = (w) => Math.min(MAX_WEIGHT, Math.max(MIN_WEIGHT, Number.isFinite(w) ? w : 1));
/** A lone window can be shrunk to a fifth of the pane, no further. */
export const MIN_FILL = 0.2;
const clampFill = (f) => Math.min(1, Math.max(MIN_FILL, Number.isFinite(f) ? f : 1));
/** The grid's share of the pane, defaulting to all of it. */
export const fillOf = (layout) => ({
...
export function isSolo(layout) {
    if (layout.columns) {
        return (layout.columns.length === 1 && layout.columns[0].items.length === 1);
    }
 ...
 */
export function resizeSolo(layout, dwFraction, dhFraction) {
    if (!isSolo(layout))
        return layout;
 ...
 */
export function defaultRowCounts(n) {
    if (n <= 0)
        return [];
...
export function layoutFromCounts(ids, counts) {
    const rows = [];
    let i = 0;
    ...
    if (i < ids.length) {
        ...
        if (rows.length > 0)
            rows[rows.length - 1].items.push(...rest);
        else
            rows.push({ weight: 1, items: rest });
    }
    const layout = { rows };
    return { ...layout, tree: treeOf(layout) };
}
/** The balanced layout for a set of nodes — what "Auto" gives you. */
export function autoLayout(ids) {
 ...
 */
export function rowsFromPositions(nodes) {
    if (nodes.length === 0)
        return { rows: [] };
    ...
    if (bands.length >= 2) {
        const columns = bands.map((band) => {
            const ordered = [...band].sort((a, b) => a.y + a.height / 2 - (b.y + b.height / 2));
    ...
    const byTop = [...nodes].sort((a, b) => a.y + a.height / 2 - (b.y + b.height / 2) || a.x - b.x);
    const bandsY = [];
    let cur = [];
    let meanCentre = 0;
    let meanHeight = 0;
    ...
    if (cur.length > 0)
        bandsY.push(cur);
    const meanRowHeight = bandsY.reduce((s, r) => s + r.reduce((m, n) => Math.max(m, n.height), 0), 0) /
        Math.max(bandsY.length, 1);
    return normalize({
        tree,
        rows: bandsY.map((band) => {
            const ordered = [...band].sort((a, b) => a.x + a.width / 2 - (b.x + b.width / 2));
 ...
 */
export function reconcile(layout, ids) {
    if (!layout || layout.rows.length === 0)
        return autoLayout(ids);
    ...
    for (const id of ids) {
        ...
        for (let r = 1; r < kept.length; r++) {
            if (kept[r].items.length <= kept[target].items.length)
                target = r;
        }
 ...
 */
export function computeBoxes(layout, paneWidth, paneHeight) {
 ...
 */
function computeColumnBoxes(layout, paneWidth, paneHeight) {
...
// ---------------------------------------------------------------------------
const cloneRows = (layout) => layout.rows.map((r) => ({ weight: r.weight, items: r.items.map((it) => ({ ...it })) }));
const cloneColumns = (layout) => layout.columns.map((c) => ({ weight: c.weight, items: c.items.map((it) => ({ ...it })) }));
/** Swap a tile with its neighbour in the same row (or column, in a column layout). */
export function moveWithinRow(layout, id, dir) {
 ...
 */
export function moveToRow(layout, id, dir) {
    const at = findTile(layout, id);
    if (!at)
        return layout;
    if (layout.columns) {
        ...
        if (creating && from.items.length === 1)
            return layout;
        const [tile] = from.items.splice(at.row, 1);
        if (creating) {
            const col = { weight: 1, items: [tile] };
            if (target < 0)
                columns.unshift(col);
            else
                columns.push(col);
 ...
 */
export function resizeTile(layout, id, delta) {
    const at = findTile(layout, id);
    if (!at)
        return layout;
    if (layout.columns) {
 ...
 */
export function resizeRow(layout, id, delta) {
    const at = findTile(layout, id);
    if (!at)
        return layout;
    if (layout.columns) {
 ...
 * same key toggles.
 */
export function toggleFullWidth(layout, id) {
    const at = findTile(layout, id);
    if (!at)
        return layout;
    if (layout.columns) {
 ...
 */
export function layoutFromColumnCounts(ids, counts) {
    const columns = [];
    let i = 0;
    for (const count of counts) {
        if (count <= 0)
        ...
        i += count;
        if (items.length > 0)
            columns.push({ weight: 1, items });
    }
    // Anything left over joins the last column rather than vanishing.
    if (i < ids.length) {
        const rest = ids.slice(i).map((id) => ({ id, weight: 1 }));
        if (columns.length > 0)
            columns[columns.length - 1].items.push(...rest);
        else
            columns.push({ weight: 1, items: rest });
    }
...
export function columnCounts(layout) {
 ...
 */
export function parseColumnCounts(text) {
    const parts = text
        .split(/[|/]+/)
        .map((p) => p.trim())
        .filter(Boolean);
