...
export function currentSpeechEpoch() {
    return speechEpoch;
}
...
export function enqueueSpeechBytes(b64Audio, mime = "audio/wav", onDrain) {
    speechQueue.push({ b64: b64Audio, mime });
    if (onDrain)
        onQueueDrain = onDrain;
    if (!queueDraining) {
        queueDraining = true;
        cloudSpeaking = true;
        playNextClip();
...
export function speakBytes(b64Audio, onEnd, mime = "audio/wav") {
    clearSpeechQueue();
...
export async function startConversation(opts) {
    ...
    processor.onaudioprocess = (e) => {
        ...
        if (rms >= SILENCE_RMS) {
            ...
            if (clockMs - lastVoiceMs >= SILENCE_HOLD_MS)
                flush();
        }
        if (speaking && clockMs - speechStartMs >= MAX_UTTER_MS)
            flush();
    };
    source.connect(processor);
    processor.connect(ctx.destination);
