...
export function parseGoalSpec(raw) {
    if (!raw || !raw.trim())
        return null;
    try {
        ...
        if (!objective && success_criteria.length === 0 && !check_method)
            return null;
        return {
            objective,
            success_criteria,
...
function parseGoalTaskEvents(raw) {
...
function parseOneGoalTask(item, index, parentId) {
    const id = typeof item.id === "string" && item.id ? item.id : `task-${index}`;
    const inherited = typeof item.parent_id === "string" && item.parent_id ? item.parent_id : parentId;
    return {
        id,
...
function flattenGoalTasks(items, parentId, start) {
    const out = [];
    items.forEach((item) => {
...
export function goalRootTasks(tasks) {
    const ids = new Set(tasks.map((t) => t.id));
    return tasks.filter((t) => !t.parent_id || !ids.has(t.parent_id));
}
export function goalTaskChildren(tasks, parentId) {
    return tasks.filter((t) => t.parent_id === parentId);
}
export function parseGoalMemory(raw) {
    try {
...
export function parseGoalSessionViews(session) {
    return {
...
//# sourceMappingURL=goalParse.js.map