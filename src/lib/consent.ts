...
export function classifyChat(text: string): ChatIntent {
  ...
  if (isExact(t, CANCEL)) return { kind: "cancel" };
  if (isExact(t, REJECT)) return { kind: "reject", feedback: raw };
  const rejected = stripPrefixList(t, REJECT_PREFIX);
  if (rejected !== null) {
    return { kind: "reject", feedback: rejected || raw };
  }
  if (isExact(t, CONTINUE)) return { kind: "continue" };
  if (isExact(t, APPROVE)) return { kind: "approve" };
  if (mentionsPlanApproval(t) && !looksLikeNewTask(t)) return { kind: "approve" };
  const rest = stripPrefixList(t, APPROVE_PREFIX);
  if (rest !== null) {
    if (!rest) return { kind: "approve" };
    if (mentionsPlan(t) || !looksLikeNewTask(rest)) return { kind: "approve" };
    return { kind: "other" };
  }
  return { kind: "other" };
}
...
export function chatApprovesPlan(text: string, previousAssistant: string): boolean {
  ...
  if (intent.kind === "continue") return looksLikePlan(previousAssistant);
  return false;
}
...
export function looksLikePlan(text: string): boolean {
  ...
  if (heading && t.length >= 160) return true;
  if (steps >= 1 && t.length >= 160 && (lower.includes("approve") || heading)) return true;
  return false;
}
...
function isExact(t: string, list: readonly string[]): boolean {
  return list.includes(t);
}

function stripPrefixList(t: string, prefixes: readonly string[]): string | null {
...
function mentionsPlan(t: string): boolean {
  return t.includes("plan") || t.includes("steps");
}

function mentionsPlanApproval(t: string): boolean {
  return mentionsPlan(t) && APPROVE_HINT.some((h) => t.includes(h)) && !looksLikeNewTask(t);
}

function looksLikeNewTask(t: string): boolean {
...
const CANCEL = ["cancel", "cancelled", "canceled", "nevermind", "never mind", "forget it", "abort"] as const;
